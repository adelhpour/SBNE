...
std::string colorStringToStr(const ColorString c) {
    switch (c) {
        ...
        case CLR_STR_DARK_CYAN:
            return "DarkCyan";
        case CLR_STR_MAGENTA:
            return "Magenta";
        case CLR_STR_DARK_MAGENTA:
            return "DarkMagenta";
        case CLR_STR_YELLOW:
            return "Yellow";
...
Veneer* veneerFromRender(RenderInformationBase& ren, Veneer* ven) {
    ...
    for (int i = 0; i < ren.getNumGradientDefinitions(); ++i) {
        ...
        for (int j = 0; j < gb->getNumGradientStops(); ++j) {
            ...
            // get offset
            if (gs->isSetOffset()) {
                RAVector r(gs->getOffset().getAbsoluteValue(), gs->getOffset().getRelativeValue());
...
VGraphicalPrimitive1D::dashArrayVec* VGraphicalPrimitive1D::stringToDashArray(const std::string& str) {
    ...
    while (!ss.eof()) {
        ...
        if (std::stringstream(temp) >> foundInteger) {
            if (!dashArray)
                dashArray = new VGraphicalPrimitive1D::dashArrayVec();
            
            dashArray->push_back(foundInteger);
        }
...
const std::string VGraphicalPrimitive1D::dashArrayToString(dashArrayVec* dav) {
    if (dav) {
        std::string str;
        for (constDashArrayIt dAIt = dav->begin(); dAIt != dav->end(); ++dAIt) {
            str.append(std::to_string(*dAIt));
            if (dAIt != dav->end() -1)
                str.append(", ");
        }
...
VTransformation2D* VRenderGroup::findElementByShape(const std::string& shape) {
    VTransformation2D* gS = NULL;
    // search in the graphical shapes to find the desired one
    for (constElementIt i = elementsBegin(); i != elementsEnd(); ++i) {
        if (stringCompare((*i)->getShapeAsString(), shape)) {
            if (!gS)
                gS = *i;
            else
                return NULL;
