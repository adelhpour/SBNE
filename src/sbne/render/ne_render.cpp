...
const std::string ne_ven_get(Veneer* ven, std::unordered_map<std::string, std::string> infoList) {
    if (ven && ven->isRenderSpecified()) {
        // colors
        if (ven->findColorById(getKeyValue(infoList, "id")))
            return ne_clr_get(ven->findColorById(getKeyValue(infoList, "id")), infoList);
        // gradients
        else if (ven->findGradientById(getKeyValue(infoList, "id")))
            return ne_grd_get(ven->findGradientById(getKeyValue(infoList, "id")), infoList);
        // line endings
        else if (ven->findLineEndingById(getKeyValue(infoList, "id")))
            return ne_le_get(ven->findLineEndingById(getKeyValue(infoList, "id")), infoList);
        // styles by id
        else if (ven->findStyleById(getKeyValue(infoList, "id")))
            return ne_stl_get(ven->findStyleById(getKeyValue(infoList, "id")), infoList);
        // styles by role list member
        else if (ven->findStyleByRoleListMember(getKeyValue(infoList, "role")))
            return ne_stl_get(ven->findStyleByRoleListMember(getKeyValue(infoList, "role")), infoList);
        // styles by type list member
        else if (ven->findStyleByTypeListMember(getKeyValue(infoList, "type")))
            return ne_stl_get(ven->findStyleByTypeListMember(getKeyValue(infoList, "type")), infoList);
        // styles by id list member
        else if (ven->findStyleByIdListMember(getKeyValue(infoList, "localId")))
            return ne_stl_get(ven->findStyleByIdListMember(getKeyValue(infoList, "localId")), infoList);
    }
    
    return std::string();
}

const std::string ne_ven_get(Veneer* ven, Network* net, std::unordered_map<std::string, std::string> infoList) {
    if (ven && ven->isRenderSpecified() && net && net->isLayoutSpecified()) {
        VGlobalStyle* style = ne_ven_findStyle(ven, net->findNetworkElement((getKeyValue(infoList, "id"))));
        
        if (style)
            return ne_stl_get(style, infoList);
    }
    
    return std::string();
}

int ne_ven_set(Veneer* ven, std::unordered_map<std::string, std::string> infoList) {
    if (ven && ven->isRenderSpecified()) {
        // colors
        if (ven->findColorById(getKeyValue(infoList, "id")))
            return ne_clr_set(ven->findColorById(getKeyValue(infoList, "id")), infoList);
        // gradients
        else if (ven->findGradientById(getKeyValue(infoList, "id")))
            return ne_grd_set(ven->findGradientById(getKeyValue(infoList, "id")), infoList);
        // line endings
        else if (ven->findLineEndingById(getKeyValue(infoList, "id")))
            return ne_le_set(ven->findLineEndingById(getKeyValue(infoList, "id")), infoList);
        // styles by id
        else if (ven->findStyleById(getKeyValue(infoList, "id")))
            return ne_stl_set(ven->findStyleById(getKeyValue(infoList, "id")), infoList);
        // styles by role list member
        else if (ven->findStyleByRoleListMember(getKeyValue(infoList, "role")))
            return ne_stl_set(ven->findStyleByRoleListMember(getKeyValue(infoList, "role")), infoList);
        // styles by type list member
        else if (ven->findStyleByTypeListMember(getKeyValue(infoList, "type")))
            return ne_stl_set(ven->findStyleByTypeListMember(getKeyValue(infoList, "type")), infoList);
        // styles by id list member
        else if (ven->findStyleByIdListMember(getKeyValue(infoList, "id")))
            return ne_stl_set(ven->findStyleByIdListMember(getKeyValue(infoList, "id")), infoList);
    }
    
    return -1;
}

int ne_ven_set(Veneer* ven, Network* net, std::unordered_map<std::string, std::string> infoList) {
    if (ven && ven->isRenderSpecified() && net && net->isLayoutSpecified()) {
        VGlobalStyle* style = ne_ven_getLocalStyleFromGlobalStyle(ven, net->findNetworkElement((getKeyValue(infoList, "id"))), ne_ven_findStyle(ven, net->findNetworkElement((getKeyValue(infoList, "id")))));
        
        if (style)
            return ne_stl_set(style, infoList);
    }
...
const std::string ne_clr_get(VColorDefinition* c, std::unordered_map<std::string, std::string> infoList) {
    if (c) {
        // value
        if (stringCompare(getKeyValue(infoList, "key"), "value"))
            return ne_clr_getValue(c);
    }
    
    return std::string();
}

int ne_clr_set(VColorDefinition* c, std::unordered_map<std::string, std::string> infoList) {
    if (c) {
        // value
        if (!getKeyValue(infoList, "value").empty())
            return ne_clr_setValue(c, getKeyValue(infoList, "value"));
    }
...
const std::string ne_grd_get(VGradientBase* g, std::unordered_map<std::string, std::string> infoList) {
    if (g) {
        // spread method
        if (stringCompare(getKeyValue(infoList, "key"), "spreadmethod"))
            return ne_grd_getSpreadMethod(g);
        // gradient stops by id
        else if (g->findStopById(getKeyValue(infoList, "stopId")))
            return ne_gstp_get(g->findStopById(getKeyValue(infoList, "stopId")), infoList);
        // gradient stops by index
        else if (isNumber(getKeyValue(infoList, "index"))) {
            if (0 <= std::stod(getKeyValue(infoList, "index")) && std::stod(getKeyValue(infoList, "index")) < g->getNumStops())
                return ne_gstp_get(g->getStops().at(std::stod(getKeyValue(infoList, "index"))), infoList);
        }
        // x1
        else if (stringCompare(getKeyValue(infoList, "key"), "x1") && ne_grd_getX1(g))
            return ne_grd_getX1(g)->toString();
        // y1
        else if (stringCompare(getKeyValue(infoList, "key"), "y1") && ne_grd_getY1(g))
            return ne_grd_getY1(g)->toString();
        // x2
        else if (stringCompare(getKeyValue(infoList, "key"), "x2") && ne_grd_getX2(g))
            return ne_grd_getX2(g)->toString();
        // y2
        else if (stringCompare(getKeyValue(infoList, "key"), "y2") && ne_grd_getY2(g))
            return ne_grd_getY2(g)->toString();
        // cx
        else if (stringCompare(getKeyValue(infoList, "key"), "cx") && ne_grd_getCx(g))
            return ne_grd_getCx(g)->toString();
        // cy
        else if (stringCompare(getKeyValue(infoList, "key"), "cy") && ne_grd_getCy(g))
            return ne_grd_getCy(g)->toString();
        // fx
        else if (stringCompare(getKeyValue(infoList, "key"), "fx") && ne_grd_getFx(g))
            return ne_grd_getFx(g)->toString();
        // fy
        else if (stringCompare(getKeyValue(infoList, "key"), "fy") && ne_grd_getFy(g))
            return ne_grd_getFy(g)->toString();
        // ratio
        else if (stringCompare(getKeyValue(infoList, "key"), "r") && ne_grd_getR(g))
            return ne_grd_getR(g)->toString();
    }
    
    return std::string();
}

int ne_grd_set(VGradientBase* g, std::unordered_map<std::string, std::string> infoList) {
    if (g) {
        // spread method
        if (!getKeyValue(infoList, "spreadmethod").empty())
            return ne_grd_setSpreadMethod(g, getKeyValue(infoList, "spreadmethod"));
        // gradient stops by id
        else if (g->findStopById(getKeyValue(infoList, "stopId")))
            return ne_gstp_set(g->findStopById(getKeyValue(infoList, "stopId")), infoList);
        // gradient stops by index
        else if (isNumber(getKeyValue(infoList, "index"))) {
            if (0 <= std::stod(getKeyValue(infoList, "index")) && std::stod(getKeyValue(infoList, "index")) < g->getNumStops())
                return ne_gstp_set(g->getStops().at(std::stod(getKeyValue(infoList, "index"))), infoList);
        }
        // x1
        else if (!getKeyValue(infoList, "x1").empty())
            return ne_grd_setX1(g, RAVector::stringToRAVector(getKeyValue(infoList, "x1")));
        // y1
        else if (!getKeyValue(infoList, "y1").empty())
            return ne_grd_setY1(g, RAVector::stringToRAVector(getKeyValue(infoList, "y1")));
        // x2
        else if (!getKeyValue(infoList, "x2").empty())
            return ne_grd_setX2(g, RAVector::stringToRAVector(getKeyValue(infoList, "x2")));
        // y2
        else if (!getKeyValue(infoList, "y2").empty())
            return ne_grd_setY2(g, RAVector::stringToRAVector(getKeyValue(infoList, "y2")));
        // cx
        else if (!getKeyValue(infoList, "cx").empty())
            return ne_grd_setCx(g, RAVector::stringToRAVector(getKeyValue(infoList, "cx")));
        // cy
        else if (!getKeyValue(infoList, "cy").empty())
            return ne_grd_setCy(g, RAVector::stringToRAVector(getKeyValue(infoList, "cy")));
        // fx
        else if (!getKeyValue(infoList, "fx").empty())
            return ne_grd_setFx(g, RAVector::stringToRAVector(getKeyValue(infoList, "fx")));
        // fy
        else if (!getKeyValue(infoList, "fy").empty())
            return ne_grd_setFy(g, RAVector::stringToRAVector(getKeyValue(infoList, "fy")));
        // ratio
        else if (!getKeyValue(infoList, "r").empty())
            return ne_grd_setR(g, RAVector::stringToRAVector(getKeyValue(infoList, "r")));
    }
...
int ne_grd_setSpreadMethod(VGradientBase* g, const std::string& spreadmethod) {
    if (g) {
        g->setSpreadMehtod(spreadmethod);
        return 0;
    }
...
bool ne_grd_isSetX1(VGradientBase* g) {
    if (g && g->isSetAsLinearGradient())
        return ((VLinearGradient*)g)->isSetX1();
...
int ne_grd_setX1(VGradientBase* g, RAVector* x1) {
    if (g && g->isSetAsLinearGradient() && x1) {
        ((VLinearGradient*)g)->setX1(RAVector(x1->a(), x1->r()));
        return 0;
    }
...
bool ne_grd_isSetY1(VGradientBase* g) {
    if (g && g->isSetAsLinearGradient())
        return ((VLinearGradient*)g)->isSetY1();
...
int ne_grd_setY1(VGradientBase* g, RAVector* y1) {
    if (g && g->isSetAsLinearGradient() && y1) {
        ((VLinearGradient*)g)->setY1(RAVector(y1->a(), y1->r()));
        return 0;
    }
...
bool ne_grd_isSetX2(VGradientBase* g) {
    if (g && g->isSetAsLinearGradient())
        return ((VLinearGradient*)g)->isSetX2();
...
int ne_grd_setX2(VGradientBase* g, RAVector* x2) {
    if (g && g->isSetAsLinearGradient() && x2) {
        ((VLinearGradient*)g)->setX2(RAVector(x2->a(), x2->r()));
        return 0;
    }
...
bool ne_grd_isSetY2(VGradientBase* g) {
    if (g && g->isSetAsLinearGradient())
        return ((VLinearGradient*)g)->isSetY2();
...
int ne_grd_setY2(VGradientBase* g, RAVector* y2) {
    if (g && g->isSetAsLinearGradient() && y2) {
        ((VLinearGradient*)g)->setY2(RAVector(y2->a(), y2->r()));
        return 0;
    }
...
bool ne_grd_isSetCx(VGradientBase* g) {
    if (g && g->isSetAsRadialGradient())
        return ((VRadialGradient*)g)->isSetCx();
...
int ne_grd_setCx(VGradientBase* g, RAVector* cx) {
    if (g && g->isSetAsRadialGradient() && cx) {
        ((VRadialGradient*)g)->setCx(RAVector(cx->a(), cx->r()));
        return 0;
    }
...
bool ne_grd_isSetCy(VGradientBase* g) {
    if (g && g->isSetAsRadialGradient())
        return ((VRadialGradient*)g)->isSetCy();
...
int ne_grd_setCy(VGradientBase* g, RAVector* cy) {
    if (g && g->isSetAsRadialGradient() && cy) {
        ((VRadialGradient*)g)->setCy(RAVector(cy->a(), cy->r()));
        return 0;
    }
...
bool ne_grd_isSetFx(VGradientBase* g) {
    if (g && g->isSetAsRadialGradient())
        return ((VRadialGradient*)g)->isSetFx();
...
int ne_grd_setFx(VGradientBase* g, RAVector* fx) {
    if (g && g->isSetAsRadialGradient() && fx) {
        ((VRadialGradient*)g)->setFx(RAVector(fx->a(), fx->r()));
        return 0;
    }
...
bool ne_grd_isSetFy(VGradientBase* g) {
    if (g && g->isSetAsRadialGradient())
        return ((VRadialGradient*)g)->isSetFy();
...
int ne_grd_setFy(VGradientBase* g, RAVector* fy) {
    if (g && g->isSetAsRadialGradient() && fy) {
        ((VRadialGradient*)g)->setFy(RAVector(fy->a(), fy->r()));
        return 0;
    }
...
bool ne_grd_isSetR(VGradientBase* g) {
    if (g && g->isSetAsRadialGradient())
        return ((VRadialGradient*)g)->isSetR();
...
int ne_grd_setR(VGradientBase* g, RAVector* r) {
    if (g && g->isSetAsRadialGradient() && r) {
        ((VRadialGradient*)g)->setR(RAVector(r->a(), r->r()));
        return 0;
    }
...
const std::string ne_gstp_get(VGradientStop* s, std::unordered_map<std::string, std::string> infoList) {
    if (s) {
        // offset
        if (stringCompare(getKeyValue(infoList, "key"), "offset") && ne_gstp_getOffset(s))
            return ne_gstp_getOffset(s)->toString();
        // stop-color
        else if (stringCompare(getKeyValue(infoList, "key"), "stop-color"))
            return ne_gstp_getColor(s);
    }
    
    return std::string();
}

int ne_gstp_set(VGradientStop* s, std::unordered_map<std::string, std::string> infoList) {
    if (s) {
        // offset
        if (!getKeyValue(infoList, "offset").empty())
            return ne_gstp_setOffset(s, RAVector::stringToRAVector(getKeyValue(infoList, "offset")));
        // stop-color
        else if (!getKeyValue(infoList, "stop-color").empty())
            return ne_gstp_setColor(s, getKeyValue(infoList, "stop-color"));
    }
...
int ne_gstp_setOffset(VGradientStop* s, RAVector* offset) {
    if (s && offset) {
        s->setOffset(RAVector(offset->a(), offset->r()));
        return 0;
    }
...
int ne_gstp_setColor(VGradientStop* s, const std::string& color) {
    if (s) {
        s->setStopColor(color);
        return 0;
    }
...
const std::string ne_le_get(VLineEnding* lE, std::unordered_map<std::string, std::string> infoList) {
    if (lE) {
        // enablerotationalmapping
        if (stringCompare(getKeyValue(infoList, "key"), "enablerotationalmapping"))
            return (ne_le_getEnableRotation(lE) ? "true" : "false");
        else {
            // bounding box
            std::string value = ne_bb_get(lE->getBox(), infoList);
            if (!value.empty())
                return value;
            
            // render group
            value = ne_grp_get(lE->getGroup(), infoList);
            if (!value.empty())
                return value;
        }
    }
...
int ne_le_set(VLineEnding* lE, std::unordered_map<std::string, std::string> infoList) {
    if (lE) {
        // enablerotationalmapping
        if (!getKeyValue(infoList, "enablerotationalmapping").empty())
            return ne_le_enableRotation(lE, stringToBoolean(getKeyValue(infoList, "enablerotationalmapping")));
        // bounding box
        else if (!ne_bb_set(lE->getBox(), infoList))
            return 0;
        // render group
        else if (!ne_grp_set(lE->getGroup(), infoList))
            return 0;
    }
...
const std::string ne_stl_get(VGlobalStyle* s, std::unordered_map<std::string, std::string> infoList) {
    if (s) {
        // render group
        return ne_grp_get(s->getGroup(), infoList);
    }
    
    return std::string();
}

int ne_stl_set(VGlobalStyle* s, std::unordered_map<std::string, std::string> infoList) {
    if (s) {
        // render group
        return ne_grp_set(s->getGroup(), infoList);
    }
...
const std::string ne_grp_get(VRenderGroup* g, std::unordered_map<std::string, std::string> infoList) {
    if (g) {
        // stroke
        if (stringCompare(getKeyValue(infoList, "key"), "stroke") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getStrokeColor(g);
        // stroke-width
        else if (stringCompare(getKeyValue(infoList, "key"), "stroke-width") && getKeyValue(infoList, "geometricShape").empty())
            return std::to_string(ne_grp_getStrokeWidth(g));
        // stroke dash array
        else if (stringCompare(getKeyValue(infoList, "key"), "stroke-dasharray") && getKeyValue(infoList, "geometricShape").empty())
            return VGraphicalPrimitive1D::dashArrayToString(ne_grp_getStrokeDashArray(g));
        // fill
        else if (stringCompare(getKeyValue(infoList, "key"), "fill") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getFillColor(g);
        // fill-rule
        else if (stringCompare(getKeyValue(infoList, "key"), "fill-rule") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getFillRule(g);
        // font-family
        else if (stringCompare(getKeyValue(infoList, "key"), "font-family") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getFontFamily(g);
        // font-size
        else if (stringCompare(getKeyValue(infoList, "key"), "font-size") && getKeyValue(infoList, "geometricShape").empty() && ne_grp_getFontSize(g))
            return ne_grp_getFontSize(g)->toString();
        // font-style
        else if (stringCompare(getKeyValue(infoList, "key"), "font-style") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getFontStyle(g);
        // font-weight
        else if (stringCompare(getKeyValue(infoList, "key"), "font-weight") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getFontWeight(g);
        // text-anchor
        else if (stringCompare(getKeyValue(infoList, "key"), "text-anchor") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getHTextAnchor(g);
        // vtext-anchor
        else if (stringCompare(getKeyValue(infoList, "key"), "vtext-anchor") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getVTextAnchor(g);
        // starthead
        else if (stringCompare(getKeyValue(infoList, "key"), "startHead") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getStartHead(g);
        // endhead
        else if (stringCompare(getKeyValue(infoList, "key"), "endHead") && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_getEndHead(g);
        // geometric shape by id
        else if (g->findElementById(getKeyValue(infoList, "geometricShape")))
            return ne_gs_get(g->findElementById(getKeyValue(infoList, "geometricShape")), infoList);
        // geometric shape by shape
        else if (g->findElementByShape(getKeyValue(infoList, "geometricShape")))
            return ne_gs_get(g->findElementByShape(getKeyValue(infoList, "geometricShape")), infoList);
    }
    
    return std::string();
}

int ne_grp_set(VRenderGroup* g, std::unordered_map<std::string, std::string> infoList) {
    if (g) {
        // stroke
        if (!getKeyValue(infoList, "stroke").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setStrokeColor(g, getKeyValue(infoList, "stroke"));
        // stroke-width
        else if (isNumber(getKeyValue(infoList, "stroke-width")) && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setStrokeWidth(g, std::stod(getKeyValue(infoList, "stroke-width")));
        // stroke dash array
        else if (!getKeyValue(infoList, "stroke-dasharray").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setStrokeDashArray(g, VGraphicalPrimitive1D::stringToDashArray(getKeyValue(infoList, "stroke-dasharray")));
        // fill
        else if (!getKeyValue(infoList, "fill").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFillColor(g, getKeyValue(infoList, "fill"));
        // fill-rule
        else if (!getKeyValue(infoList, "fill-rule").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFillRule(g, getKeyValue(infoList, "fill-rule"));
        // font-family
        else if (!getKeyValue(infoList, "font-family").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFontFamily(g, getKeyValue(infoList, "font-family"));
        // font-size
        else if (!getKeyValue(infoList, "font-size").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFontSize(g, RAVector::stringToRAVector(getKeyValue(infoList, "font-size")));
        // font-style
        else if (!getKeyValue(infoList, "font-style").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFontStyle(g, getKeyValue(infoList, "font-style"));
        // font-weight
        else if (!getKeyValue(infoList, "font-weight").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setFontWeight(g, getKeyValue(infoList, "font-weight"));
        // text-anchor
        else if (!getKeyValue(infoList, "text-anchor").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setHTextAnchor(g, getKeyValue(infoList, "text-anchor"));
        // vtext-anchor
        else if (!getKeyValue(infoList, "vtext-anchor").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setVTextAnchor(g, getKeyValue(infoList, "vtext-anchor"));
        // startHead
        else if (!getKeyValue(infoList, "startHead").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setStartHead(g, getKeyValue(infoList, "startHead"));
        // endHead
        else if (!getKeyValue(infoList, "endHead").empty() && getKeyValue(infoList, "geometricShape").empty())
            return ne_grp_setEndHead(g, getKeyValue(infoList, "endHead"));
        // geometric shape by id
        else if (g->findElementById(getKeyValue(infoList, "geometricShape")))
            return ne_gs_set(g->findElementById(getKeyValue(infoList, "geometricShape")), infoList);
        // geometric shape by shape
        else if (g->findElementByShape(getKeyValue(infoList, "geometricShape")))
            return ne_gs_set(g->findElementByShape(getKeyValue(infoList, "geometricShape")), infoList);
    }
...
const std::string ne_gs_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS) {
        // stroke
        if (stringCompare(getKeyValue(infoList, "key"), "stroke"))
            return ne_gs_getStrokeColor(gS);
        // stroke-width
        else if (stringCompare(getKeyValue(infoList, "key"), "stroke-width"))
            return std::to_string(ne_gs_getStrokeWidth(gS));
        // stroke dash array
        else if (stringCompare(getKeyValue(infoList, "key"), "stroke-dasharray"))
            return VGraphicalPrimitive1D::dashArrayToString(ne_gs_getStrokeDashArray(gS));
        // fill
        else if (stringCompare(getKeyValue(infoList, "key"), "fill"))
            return ne_gs_getFillColor(gS);
        // fill-rule
        else if (stringCompare(getKeyValue(infoList, "key"), "fill-rule"))
            return ne_gs_getFillRule(gS);
        // exclusive attributes
        else {
            switch (gS->getShape()) {
                // image
                case 0:
                    return ne_img_get(gS, infoList);
                    
                // rendercurve
                case 1:
                    return ne_rc_get(gS, infoList);
                    
                // rectangle
                case 3:
                    return ne_rec_get(gS, infoList);
                    
                // ellipse
                case 4:
                    return ne_elp_get(gS, infoList);
                    
                // polygon
                case 5:
                    return ne_plg_get(gS, infoList);
...
int ne_gs_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS) {
        // stroke
        if (!getKeyValue(infoList, "stroke").empty())
            return ne_gs_setStrokeColor(gS, getKeyValue(infoList, "stroke"));
        // stroke-width
        else if (isNumber(getKeyValue(infoList, "stroke-width")))
            return ne_gs_setStrokeWidth(gS, std::stod(getKeyValue(infoList, "stroke-width")));
        // stroke dash array
        else if (!getKeyValue(infoList, "stroke-dasharray").empty())
            return ne_gs_setStrokeDashArray(gS, VGraphicalPrimitive1D::stringToDashArray(getKeyValue(infoList, "stroke-dasharray")));
        // fill
        else if (!getKeyValue(infoList, "fill").empty())
            return ne_gs_setFillColor(gS, getKeyValue(infoList, "fill"));
        // fill-rule
        else if (!getKeyValue(infoList, "fill-rule").empty())
            return ne_gs_setFillRule(gS, getKeyValue(infoList, "fill-rule"));
        // exclusive attributes
        else {
            switch (gS->getShape()) {
                // image
                case 0:
                    return ne_img_set(gS, infoList);
                    
                // rendercurve
                case 1:
                    return ne_rc_set(gS, infoList);
                    
                // rectangle
                case 3:
                    return ne_rec_set(gS, infoList);
                    
                // ellipse
                case 4:
                    return ne_elp_set(gS, infoList);
                    
                // polygon
                case 5:
                    return ne_plg_set(gS, infoList);
...
const std::string ne_img_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 0) {
        // x
        if (stringCompare(getKeyValue(infoList, "key"), "x") && ne_img_getPositionX(gS))
            return ne_img_getPositionX(gS)->toString();
        // y
        else if (stringCompare(getKeyValue(infoList, "key"), "y") && ne_img_getPositionY(gS))
            return ne_img_getPositionY(gS)->toString();
        // width
        else if (stringCompare(getKeyValue(infoList, "key"), "width") && ne_img_getDimensionWidth(gS))
            return ne_img_getDimensionWidth(gS)->toString();
        // height
        else if (stringCompare(getKeyValue(infoList, "key"), "height") && ne_img_getDimensionHeight(gS))
            return ne_img_getDimensionHeight(gS)->toString();
        // href
        else if (stringCompare(getKeyValue(infoList, "key"), "href"))
            return ne_img_getHref(gS);
    }
    
    return std::string();
}

int ne_img_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 0) {
        // x
        if (!getKeyValue(infoList, "x").empty())
            return ne_img_setPositionX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "x")));
        // y
        else if (!getKeyValue(infoList, "y").empty())
            return ne_img_setPositionY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "y")));
        // width
        else if (!getKeyValue(infoList, "width").empty())
            return ne_img_setDimensionWidth(gS, RAVector::stringToRAVector(getKeyValue(infoList, "width")));
        // height
        else if (!getKeyValue(infoList, "height").empty())
            return ne_img_setDimensionHeight(gS, RAVector::stringToRAVector(getKeyValue(infoList, "height")));
        // href
        else if (!getKeyValue(infoList, "href").empty())
            return ne_img_setHref(gS, getKeyValue(infoList, "href"));
    }
...
const std::string ne_rc_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 1) {
        // vertices
        if (isNumber(getKeyValue(infoList, "index")))
            return ne_vrx_get(ne_rc_getVertex(gS, std::stod(getKeyValue(infoList, "index"))), infoList);
        // starthead
        else if (stringCompare(getKeyValue(infoList, "key"), "startHead"))
            return ne_rc_getStartHead(gS);
        // endthead
        else if (stringCompare(getKeyValue(infoList, "key"), "endHead"))
            return ne_rc_getEndHead(gS);
    }
    
    return std::string();
}

int ne_rc_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 1) {
        // vertices
        if (isNumber(getKeyValue(infoList, "index")))
            return ne_vrx_set(ne_rc_getVertex(gS, std::stod(getKeyValue(infoList, "index"))), infoList);
        // starthead
        else if(!getKeyValue(infoList, "startHead").empty())
            return ne_rc_setStartHead(gS, getKeyValue(infoList, "startHead"));
        // endhead
        else if(!getKeyValue(infoList, "endHead").empty())
            return ne_rc_setStartHead(gS, getKeyValue(infoList, "endHead"));
    }
...
const std::string ne_txt_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 2) {
        // x
        if (stringCompare(getKeyValue(infoList, "key"), "x") && ne_txt_getPositionX(gS))
            return ne_txt_getPositionX(gS)->toString();
        // y
        else if (stringCompare(getKeyValue(infoList, "key"), "y") && ne_txt_getPositionY(gS))
            return ne_txt_getPositionY(gS)->toString();
        // font-family
        else if (stringCompare(getKeyValue(infoList, "key"), "font-family"))
            return ne_txt_getFontFamily(gS);
        // font-size
        else if (stringCompare(getKeyValue(infoList, "key"), "font-size") && ne_txt_getFontSize(gS))
            return ne_txt_getFontSize(gS)->toString();
        // font-style
        else if (stringCompare(getKeyValue(infoList, "key"), "font-style"))
            return ne_txt_getFontStyle(gS);
        // font-weight
        else if (stringCompare(getKeyValue(infoList, "key"), "font-weight"))
            return ne_txt_getFontWeight(gS);
        // text-anchor
        else if (stringCompare(getKeyValue(infoList, "key"), "text-anchor"))
            return ne_txt_getHTextAnchor(gS);
        // vtext-anchor
        else if (stringCompare(getKeyValue(infoList, "key"), "vtext-anchor"))
            return ne_txt_getVTextAnchor(gS);
    }
    
    return std::string();
}

int ne_txt_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 2) {
        // x
        if (!getKeyValue(infoList, "x").empty())
            return ne_txt_setPositionX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "x")));
        // y
        else if (!getKeyValue(infoList, "y").empty())
            return ne_txt_setPositionY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "y")));
        else if (!getKeyValue(infoList, "font-family").empty())
            return ne_txt_setFontFamily(gS, getKeyValue(infoList, "font-family"));
        // font-size
        else if (!getKeyValue(infoList, "font-size").empty())
            return ne_txt_setFontSize(gS, RAVector::stringToRAVector(getKeyValue(infoList, "font-size")));
        // font-style
        else if (!getKeyValue(infoList, "font-style").empty())
            return ne_txt_setFontStyle(gS, getKeyValue(infoList, "font-style"));
        // font-weight
        else if (!getKeyValue(infoList, "font-weight").empty())
            return ne_txt_setFontWeight(gS, getKeyValue(infoList, "font-weight"));
        // text-anchor
        else if (!getKeyValue(infoList, "text-anchor").empty())
            return ne_txt_setHTextAnchor(gS, getKeyValue(infoList, "text-anchor"));
        // vtext-anchor
        else if (!getKeyValue(infoList, "vtext-anchor").empty())
            return ne_txt_setVTextAnchor(gS, getKeyValue(infoList, "vtext-anchor"));
    }
...
const std::string ne_rec_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 3) {
        // x
        if (stringCompare(getKeyValue(infoList, "key"), "x") && ne_rec_getPositionX(gS))
            return ne_rec_getPositionX(gS)->toString();
        // y
        else if (stringCompare(getKeyValue(infoList, "key"), "y") && ne_rec_getPositionY(gS))
            return ne_rec_getPositionY(gS)->toString();
        // width
        else if (stringCompare(getKeyValue(infoList, "key"), "width") && ne_rec_getDimensionWidth(gS))
            return ne_rec_getDimensionWidth(gS)->toString();
        // height
        else if (stringCompare(getKeyValue(infoList, "key"), "height") && ne_rec_getDimensionHeight(gS))
            return ne_rec_getDimensionHeight(gS)->toString();
        // rx
        else if (stringCompare(getKeyValue(infoList, "key"), "rx") && ne_rec_getCornerCurvatureRX(gS))
            return ne_rec_getCornerCurvatureRX(gS)->toString();
        // ry
        else if (stringCompare(getKeyValue(infoList, "key"), "ry") && ne_rec_getCornerCurvatureRY(gS))
            return ne_rec_getCornerCurvatureRY(gS)->toString();
        // ratio
        else if (stringCompare(getKeyValue(infoList, "key"), "ratio"))
            return std::to_string(ne_rec_getRatio(gS));
    }
    
    return std::string();
}

int ne_rec_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 3) {
        // x
        if (!getKeyValue(infoList, "x").empty())
            return ne_rec_setPositionX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "x")));
        // y
        else if (!getKeyValue(infoList, "y").empty())
            return ne_rec_setPositionY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "y")));
        // width
        else if (!getKeyValue(infoList, "width").empty())
            return ne_rec_setDimensionWidth(gS, RAVector::stringToRAVector(getKeyValue(infoList, "width")));
        // height
        else if (!getKeyValue(infoList, "height").empty())
            return ne_rec_setDimensionHeight(gS, RAVector::stringToRAVector(getKeyValue(infoList, "height")));
        // rx
        else if (!getKeyValue(infoList, "rx").empty())
            return ne_rec_setCornerCurvatureRX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "rx")));
        // ry
        else if (!getKeyValue(infoList, "ry").empty())
            return ne_rec_setCornerCurvatureRY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "ry")));
        // ratio
        else if (isNumber(getKeyValue(infoList, "ratio")))
            return ne_rec_setRatio(gS, std::stod(getKeyValue(infoList, "ratio")));
    }
...
const std::string ne_elp_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 4) {
        // cx
        if (stringCompare(getKeyValue(infoList, "key"), "cx") && ne_elp_getPositionCX(gS))
            return ne_elp_getPositionCX(gS)->toString();
        // cy
        else if (stringCompare(getKeyValue(infoList, "key"), "cy") && ne_elp_getPositionCY(gS))
            return ne_elp_getPositionCY(gS)->toString();
        // rx
        else if (stringCompare(getKeyValue(infoList, "key"), "rx") && ne_elp_getDimensionRX(gS))
            return ne_elp_getDimensionRX(gS)->toString();
        // ry
        else if (stringCompare(getKeyValue(infoList, "key"), "ry") && ne_elp_getDimensionRY(gS))
            return ne_elp_getDimensionRY(gS)->toString();
        // ratio
        else if (stringCompare(getKeyValue(infoList, "key"), "ratio"))
            return std::to_string(ne_elp_getRatio(gS));
    }
    
    return std::string();
}

int ne_elp_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 4) {
        // cx
        if (!getKeyValue(infoList, "cx").empty())
            return ne_elp_setPositionCX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "cx")));
        // cy
        if (!getKeyValue(infoList, "cy").empty())
            return ne_elp_setPositionCY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "cy")));
        // rx
        if (!getKeyValue(infoList, "rx").empty())
            return ne_elp_setDimensionRX(gS, RAVector::stringToRAVector(getKeyValue(infoList, "rx")));
        // ry
        if (!getKeyValue(infoList, "ry").empty())
            return ne_elp_setDimensionRY(gS, RAVector::stringToRAVector(getKeyValue(infoList, "ry")));
        // ratio
        else if (isNumber(getKeyValue(infoList, "ratio")))
            return ne_elp_setRatio(gS, std::stod(getKeyValue(infoList, "ratio")));
    }
...
const std::string ne_plg_get(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 5 && isNumber(getKeyValue(infoList, "index")))
        return ne_vrx_get(ne_plg_getVertex(gS, std::stod(getKeyValue(infoList, "index"))), infoList);
    
    return std::string();
}

int ne_plg_set(VTransformation2D* gS, std::unordered_map<std::string, std::string> infoList) {
    if (gS && gS->getShape() == 5 && isNumber(getKeyValue(infoList, "index")))
        return ne_vrx_set(ne_plg_getVertex(gS, std::stod(getKeyValue(infoList, "index"))), infoList);
...
const std::string ne_vrx_get(RenPoint* rp, std::unordered_map<std::string, std::string> infoList) {
    if (rp) {
        // renderpoint
        if (stringCompare(getKeyValue(infoList, "point"), "renderpoint"))
            return ne_rp_get(ne_vrx_getRenderPoint(rp), infoList);
        // basepoint1
        else if (stringCompare(getKeyValue(infoList, "point"), "basepoint1"))
            return ne_rp_get(ne_vrx_getBasePoint1(rp), infoList);
        // basepoint2
        else if (stringCompare(getKeyValue(infoList, "point"), "basepoint2"))
            return ne_rp_get(ne_vrx_getBasePoint2(rp), infoList);
    }
    
    return std::string();
}

int ne_vrx_set(RenPoint* rp, std::unordered_map<std::string, std::string> infoList) {
    if (rp) {
        RPoint* point = NULL;
        // renderpoint
        if (stringCompare(getKeyValue(infoList, "point"), "renderpoint")) {
            point = new RPoint(rp->getRPoint());
            if (!ne_rp_set(point, infoList)) {
                rp->setRPoint(RPoint(point->x(), point->y()));
                delete point;
                return 0;
            }
        }
        // render cubic bezier
        else if (ne_vrx_isRenderCubicBezier(rp)) {
            // basepoint1
            if (stringCompare(getKeyValue(infoList, "point"), "basepoint1")) {
                point = new RPoint(((RCubicBezier*)(rp))->getBasePoint1());
                if (!ne_rp_set(point, infoList)) {
                    ((RCubicBezier*)(rp))->setBasePoint1(RPoint(point->x(), point->y()));
                    delete point;
                    return 0;
                }
            }
            // basepoint2
                ...
                point = new RPoint(((RCubicBezier*)(rp))->getBasePoint1());
                if (!ne_rp_set(point, infoList)) {
                    ((RCubicBezier*)(rp))->setBasePoint2(RPoint(point->x(), point->y()));
                    delete point;
                    return 0;
                }
            }
...
const std::string ne_rp_get(RPoint* point, std::unordered_map<std::string, std::string> infoList) {
    if (point) {
        // x
        if (stringCompare(getKeyValue(infoList, "key"), "x") && ne_rp_getX(point))
            return ne_rp_getX(point)->toString();
        // y
        else if (stringCompare(getKeyValue(infoList, "key"), "y") && ne_rp_getY(point))
            return ne_rp_getY(point)->toString();
    }
    
    return std::string();
}

int ne_rp_set(RPoint* point, std::unordered_map<std::string, std::string> infoList) {
    if (point) {
        // x
        if (!getKeyValue(infoList, "x").empty())
            return ne_rp_setX(point, RAVector::stringToRAVector(getKeyValue(infoList, "x")));
        // y
        else if (!getKeyValue(infoList, "y").empty())
            return ne_rp_setY(point, RAVector::stringToRAVector(getKeyValue(infoList, "y")));
    }
