...
VGlobalStyle* ne_ven_getStyle(Veneer* ven, const unsigned int& index) {
    if (ven) {
    ...
    if (s) {
        ...
        if (type < NUM_StyleType) {
            ...
            if (styleTypeToStr(type) == "COMPARTMENTGLYPH") {
                ...
                // set fill color
                ne_grp_setFillColor(group, colorStringToStr(CLR_STR_CYAN));
...
VTransformation2D* ne_grp_addGeometricShape(VRenderGroup* g, const RenderGroupElementShape rGEShape, const bool& forLineEnding, const int& numberOfPolygonVertices, const std::string& imageHref) {
    if (g) {
        ...
        ne_gs_setShapeFeatures(gS, numberOfPolygonVertices, imageHref, forLineEnding);
...
int ne_gs_setShapeFeatures(VTransformation2D* gS, const int& numberOfPolygonVertices, const std::string& imageHref, const bool& forLineEnding) {
    if (gS) {
        RAVector* rAV = new RAVector();
        switch (gS->getShape()) {
            ...
            case 5: {
                ...
                switch (numberOfPolygonVertices) {
                    // triangle
                    case 3:
                        if (!forLineEnding) {
...
RAVector* ne_rp_getX(RPoint* rP) {
    RAVector* rAVector = NULL;
    if (rP)
        rAVector = new RAVector(rP->x());
    
    return rAVector;
}

int ne_rp_setX(RPoint* rP, RAVector* x) {
    if (rP && x) {
        rP->setX(RAVector(x->a(), x->r()));
        return 0;
    }
...
RAVector* ne_rp_getY(RPoint* rP) {
    RAVector* rAVector = NULL;
    if (rP)
        rAVector = new RAVector(rP->y());
    
    return rAVector;
}

int ne_rp_setY(RPoint* rP, RAVector* y) {
    if (rP && y) {
        rP->setY(RAVector(y->a(), y->r()));
        return 0;
    }
...
ne_renderInfo* ne_doc_processRenderInfo(SBMLDocument* doc) {
    ne_renderInfo* r = new ne_renderInfo();
...
Veneer* ne_ri_addDefaultRenderFeaturesToVeneer(ne_renderInfo* r) {
    if (r) {
...
int ne_ri_setVeneer(ne_renderInfo* r, Veneer* ven) {
    if (ven) {
        r->ven = ven;
        return 0;
    }
...
Veneer* ne_ri_getVeneer(ne_renderInfo* r) {
    if (r->ven)
        return (Veneer*)(r->ven);
...
SBMLDocument* ne_doc_populateSBMLdocWithRenderInfo(SBMLDocument* doc, ne_renderInfo* r) {
    if (doc) {
        GlobalRenderInformation* gri = NULL;
        LocalRenderInformation* lri = NULL;
