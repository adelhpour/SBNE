...
void ne_li_setNetwork(ne_layoutInfo* l, Network* net) {
    if (net)
        l->net = net;
    else
        std::cerr << "No network to set to layout info\n";
}

Network* ne_li_getNetwork(ne_layoutInfo* l) {
    if (l->net)
        return (Network*)(l->net);
    else
        std::cerr << "No network to get layout info from\n";
    
    return NULL;
}

Network* ne_li_addLayoutFeaturesToNetowrk(ne_layoutInfo* l) {
    // get network
...
int ne_go_unSetBoundingBox(NGraphicalObject* gO, const bool& removeBoundingBox) {
    if (gO) {
        gO->unSetBox(removeBoundingBox);
        return 0;
    }
...
SReferenceVec ne_net_getSpeciesReferences(NReaction* r) {
    if (r)
...
int ne_rxn_unSetCurve(NReaction* r, const bool& removeCurve) {
    if (r) {
        r->unSetCurve(removeCurve);
        return 0;
    }
...
ne_layoutInfo* ne_doc_processLayoutInfo(SBMLDocument* doc) {
    ne_layoutInfo* l = new ne_layoutInfo();
...
SBMLDocument* ne_doc_populateSBMLdocWithLayoutInfo(SBMLDocument* doc ,ne_layoutInfo* l) {
    if (doc && l) {
